...
def normalize_template(value: str | None) -> str:
    template = upper_text(value) or "STANDARD"
...
def create_sales_invoice(
    ...
    customer_code = upper_text(data.get("customer_code"))
    ...
    for index, ln in enumerate(lines, start=1):
        item_code = upper_text(ln.get("item_code"))
        ...
        if qty <= 0:
            raise HTTPException(
                status_code=400,
                detail=f"Line {index}: Qty must be greater than 0",
            )

        if rate < 0:
            raise HTTPException(
                status_code=400,
                detail=f"Line {index}: Rate cannot be negative",
            )

        if line_tax_percent < 0:
            raise HTTPException(
                status_code=400,
                detail=f"Line {index}: Tax percent cannot be negative",
            )
...
def update_sales_invoice(
    ...
    obj.due_date = payload.due_date
    obj.customer_code = upper_text(payload.customer_code)
    ...
    for index, ln in enumerate(payload.lines, start=1):
        item_code = upper_text(ln.item_code)
