...
function sortLatestFirst(rows, tab) {
  return [...rows].sort((a, b) => {
    const dateA = safeISO(getDocDate(a, tab));
    const dateB = safeISO(getDocDate(b, tab));
...
export default function Ledger() {
  ...
  const monthlyTrend = useMemo(() => {
    ...
    filteredRows.forEach((r) => {
      const rawDate = getDocDate(r, tab);
      const iso = safeISO(rawDate);
      if (!iso) return;
                ...
                filteredRows.map((row) => {
                  ...
                  return (
                    <tr key={docNo} style={tr}>
                      ...
                      <td style={td}>{formatDate(dateValue)}</td>
                      <td style={td}>{formatDate(row.due_date)}</td>
